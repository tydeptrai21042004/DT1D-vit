...
# Run from the root of an already-cloned corrected DT1D-vit repository.
...
required = ['active_offsets_for_repo_schema','preflight_yacs_merge','[SKIP] compatible completed run','[RERUN] incompatible completed run']
missing = [x for x in required if x not in s]
if missing:
    raise SystemExit('ERROR: this is not the corrected resume-safe repo. Missing: ' + repr(missing))
